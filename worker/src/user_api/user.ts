...
export default {
  ...
  register: async (c: Context<HonoCustomType>) => {
    ...
    const { success } = await c.env.DB.prepare(
      ...
      .bind(email, password, JSON.stringify(userInfo), password, JSON.stringify(userInfo))
      .run();
    if (!success) {
    ...
    // find user_id
    const user_id = await c.env.DB.prepare(`SELECT id FROM users where user_email = ?`)
      .bind(email)
  ...
  login: async (c: Context<HonoCustomType>) => {
    ...
    const { id: user_id, password: dbPassword } =
      (await c.env.DB.prepare(`SELECT id, password FROM users where user_email = ?`)
        .bind(email)
        .first()) || {};
    if (!dbPassword) {
