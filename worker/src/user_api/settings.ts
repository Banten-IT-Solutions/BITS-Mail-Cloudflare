...
export default {
  openSettings: async (c: Context<HonoCustomType>) => {
    ...
    try {
      const oauth2Settings = await getJsonSetting<UserOauth2Settings[]>(
        c,
        CONSTANTS.OAUTH2_SETTINGS_KEY
      );
  ...
  settings: async (c: Context<HonoCustomType>) => {
    ...
    const is_admin = c.env.ADMIN_USER_ROLE && c.env.ADMIN_USER_ROLE === user_role?.role;
    const access_token = user_role?.role
      ? await Jwt.sign(
          {
