...
export default {
  ...
  oauth2Login: async (c: Context<HonoCustomType>) => {
    ...
    const res = await fetch(setting.accessTokenURL, {
      method: 'POST',
      body:
        setting.accessTokenFormat === 'json'
    ...
    if (!res.ok) {
      console.error(
        `Failed to get access token: ${res.status} ${res.statusText} ${await res.text()}`
      );
    ...
    if (!userRes.ok) {
      console.error(
        `Failed to get user info: ${userRes.status} ${userRes.statusText} ${await userRes.text()}`
      );
    ...
    if (setting.enableMailAllowList && !setting.mailAllowList?.includes(mailDomain)) {
      return c.text(
        `${msgs.UserMailDomainMustInMsg} ${JSON.stringify(setting.mailAllowList, null, 2)}`,
        400
      );
    }
    ...
    const { success } = await c.env.DB.prepare(
      ...
      .bind(email, JSON.stringify(userInfo))
      .run();
    if (!success) {
      return c.text(msgs.FailedToRegisterMsg, 500);
    }
    const { id: user_id } =
      (await c.env.DB.prepare(`SELECT id FROM users where user_email = ?`).bind(email).first()) ||
      {};
    if (!user_id) {
