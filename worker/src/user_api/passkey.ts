...
export default {
  ...
  renamePassKey: async (c: Context<HonoCustomType>) => {
    ...
    const { success } = await c.env.DB.prepare(
      ...
      .bind(passkey_name, user.user_id, passkey_id)
      .run();
  ...
  registerResponse: async (c: Context<HonoCustomType>) => {
    ...
    const { success } = await c.env.DB.prepare(
      ...
      .bind(user.user_id, passkey_name, credentialID, JSON.stringify(newPasskey), counter)
      .run();
  ...
  authenticateResponse: async (c: Context<HonoCustomType>) => {
    ...
    if (authenticationInfo) {
      ...
      await c.env.DB.prepare(`UPDATE user_passkeys SET counter = ? WHERE passkey_id = ?`)
        .bind(newCounter, passkey_id)
        .run();
    }
    ...
    // return jwt
    const { user_email } =
      (await c.env.DB.prepare(`SELECT user_email FROM users WHERE id = ?`)
        .bind(user_id)
