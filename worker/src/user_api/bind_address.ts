...
const UserBindAddressModule = {
  ...
  bindByID: async (
    ...
    const db_user_id = await c.env.DB.prepare(`SELECT id FROM users where id = ?`)
      .bind(user_id)
      .first('id');
    if (!db_user_id) {
  ...
  getBindedAddressJwt: async (c: Context<HonoCustomType>) => {
    ...
    const db_user_id = await c.env.DB.prepare(
      ...
      .bind(address_id, user_id)
      .first('user_id');
    if (!db_user_id) {
      return c.text(msgs.AddressNotBindedMsg, 400);
    }
    // generate jwt
    const name = await c.env.DB.prepare(`SELECT name FROM address WHERE id = ? `)
      .bind(address_id)
      .first('name');
  ...
  transferAddress: async (c: Context<HonoCustomType>) => {
    ...
    // check if address exists
    const address = await c.env.DB.prepare(`SELECT name FROM address where id = ?`)
      .bind(address_id)
      .first<string>('name');
    if (!address) {
      return c.text(msgs.AddressNotFoundMsg, 400);
    ...
    const target_user_id = await c.env.DB.prepare(`SELECT id FROM users where user_email = ?`)
    ...
    // find new address id
    const new_address_id = await c.env.DB.prepare(`SELECT id FROM address WHERE name = ?`)
      .bind(address)
      .first<number | null | undefined>('id');
    if (!new_address_id) {
    ...
    try {
      const { success } = await c.env.DB.prepare(
        ...
        .bind(target_user_id, new_address_id)
        .run();
      if (!success) {
        return c.text(msgs.OperationFailedMsg, 500);
      }
