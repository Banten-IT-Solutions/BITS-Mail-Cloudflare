...
const API_PATHS = ['/api/', '/open_api/', '/user_api/', '/admin/', '/telegram/', '/external/'];
...
app.use('/*', async (c, next) => {
  ...
  if (
    ...
    if (reqIp && c.env.RATE_LIMITER) {
      const { success } = await c.env.RATE_LIMITER.limit({ key: `${c.req.path}|${reqIp}` });
      if (!success) {
...
const checkUserPayload = async (c: Context<HonoCustomType>): Promise<void> => {
  try {
...
const checkoutUserRolePayload = async (c: Context<HonoCustomType>): Promise<void> => {
  try {
...
app.use('/api/*', async (c, next) => {
  ...
  if (c.req.path.startsWith('/api/settings') || c.req.path.startsWith('/api/send_mail')) {
