...
const parseSendMailLimitConfig = (value: unknown): SendMailLimitConfig | null => {
  ...
  const monthlyValid = config.monthlyEnabled
    ...
    : config.monthlyLimit === null ||
      typeof config.monthlyLimit === 'undefined' ||
      monthlyLimit !== null;
...
export const getSendMailLimitConfigToSave = (value: unknown): SendMailLimitConfig | null => {
...
export const getSendMailLimitConfig = async (
  ...
  return getSendMailLimitConfigToSave(
    getJsonObjectValue<SendMailLimitConfig>(
...
const getCount = async (c: Context<HonoCustomType>, key: string): Promise<number> => {
...
export const ensureSendMailLimit = async (c: Context<HonoCustomType>): Promise<void> => {
  try {
...
const increaseCount = async (c: Context<HonoCustomType>, key: string): Promise<void> => {
...
export const increaseSendMailLimitCount = async (c: Context<HonoCustomType>): Promise<void> => {
  try {
    const config = await getSendMailLimitConfig(c);
    if (!config || (!config.dailyEnabled && !config.monthlyEnabled)) {
      return;
