...
const getMail = async (c: Context<HonoCustomType>) => {
  ...
  const result = await c.env.DB.prepare(`SELECT * FROM raw_mails where id = ? and address = ?`)
    .bind(mail_id, address)
    .first();
...
const deleteMail = async (c: Context<HonoCustomType>) => {
  ...
  const { success } = await c.env.DB.prepare(`DELETE FROM raw_mails WHERE address = ? and id = ? `)
...
const clearInbox = async (c: Context<HonoCustomType>) => {
  ...
  const { address } = c.get('jwtPayload');
  const { success } = await c.env.DB.prepare(`DELETE FROM raw_mails WHERE address = ?`)
    .bind(address)
    .run();
  if (!success) {
...
const clearSentItems = async (c: Context<HonoCustomType>) => {
  ...
  const { address } = c.get('jwtPayload');
  const { success } = await c.env.DB.prepare(`DELETE FROM sendbox WHERE address = ?`)
    .bind(address)
    .run();
  if (!success) {
