...
const sendMailByResend = async (
  ...
  const token =
    c.env[`RESEND_TOKEN_${mailDomain.replace(/\./g, '_').toUpperCase()}`] || c.env.RESEND_TOKEN;
...
const sendMailBySmtp = async (
  ...
  await WorkerMailer.send(smtpOptions, {
    from: {
...
api.delete('/api/sendbox/:id', async c => {
  ...
  const { success } = await c.env.DB.prepare(`DELETE FROM sendbox WHERE address = ? and id = ? `)
    .bind(address, id)
    .run();
  return c.json({
    success: success,
