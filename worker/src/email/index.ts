...
async function email(message: ForwardableEmailMessage, env: Bindings, ctx: ExecutionContext) {
  ...
  // check if junk mail
  try {
    const is_junk = await check_if_junk_mail(
      env,
      message.to,
      parsedEmailContext,
      ...
      if (compressed) {
        try {
          ({ success } = await env.DB.prepare(
            ...
            .bind(message.from, message.to, compressed, message_id)
            .run());
        } catch (dbError) {
          ...
          if (errMsg.includes('raw_blob') || errMsg.includes('no such column')) {
            ...
            ({ success } = await env.DB.prepare(
              ...
              .bind(message.from, message.to, parsedEmailContext.rawEmail, message_id)
              .run());
          } else {
            throw dbError;
          }
