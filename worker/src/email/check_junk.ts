...
export const check_if_junk_mail = async (
  ...
  if (
    checkListWhenExist?.some(
      checkName =>
        existList.includes(checkName.toLowerCase()) && !passedList.includes(checkName.toLowerCase())
    )
  ) {
    return true;
  }
  ...
  return forcePassList.some(checkName => !passedList.includes(checkName.toLowerCase()));
};
