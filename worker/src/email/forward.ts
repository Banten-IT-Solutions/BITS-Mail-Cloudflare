...
async function forwardByRules(message: ForwardableEmailMessage, env: Bindings): Promise<void> {
  try {
    ...
    const subdomainForwardAddressList =
      getJsonObjectValue<SubdomainForwardAddressList[]>(env.SUBDOMAIN_FORWARD_ADDRESS_LIST) || [];
...
export { forwardEmail, forwardToGlobalAddresses, forwardByRules, matchSourcePatterns };
