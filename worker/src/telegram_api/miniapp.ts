...
const checkTelegramAuth = async (c: Context<HonoCustomType>, initData: string): Promise<string> => {
  ...
  const dataToCheck = [...initDataObj.entries()]
...
async function getMail(c: Context<HonoCustomType>): Promise<Response> {
  ...
  try {
    if (checkIsAdmin(c)) {
      const result = await c.env.DB.prepare(`SELECT * FROM raw_mails where id = ?`)
        .bind(mailId)
        .first();
      if (!result) {
    ...
    const superUser =
      settings?.enableGlobalMailPush && settings?.globalMailPushList.includes(userId);
    if (!superUser) {
