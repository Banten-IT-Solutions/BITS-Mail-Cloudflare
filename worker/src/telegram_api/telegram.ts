...
import {
  ...
  jwtListToAddressData,
  tgUserNewAddress,
  unbindTelegramAddress,
  unbindTelegramByAddress,
} from './common';
...
const getTgMessages = async (
  ...
  const uid =
    userId || ctx?.message?.from?.id?.toString() || ctx?.callbackQuery?.from?.id?.toString();
  if (uid) {
    const savedLang = await c.env.KV.get(`${CONSTANTS.TG_KV_PREFIX}:lang:${uid}`);
    if (savedLang) {
      return i18n.getMessages(savedLang);
    }
  }
  return i18n.getMessages(c.env.DEFAULT_LANG || 'zh');
};
...
const COMMANDS = [
  ...
  },
  {
    command: 'new',
    description:
      '新建邮箱, /new <name>@<domain>, name[a-z0-9]有效, 为空随机生成, @domain可选 | Create address, /new <name>@<domain>, name[a-z0-9] valid, empty=random, @domain optional',
  },
  {
    command: 'address',
  ...
  },
  {
    command: 'mails',
    description:
      '查看邮件, /mails <邮箱地址>, 不输入地址默认第一个 | View mails, /mails <address>, default first if empty',
  },
  {
...
export function newTelegramBot(c: Context<HonoCustomType>, token: string): Telegraf {
  ...
  bot.use(async (ctx, next) => {
    ...
    if (settings?.enableAllowList && !settings.allowList.includes(userId.toString())) {
      const msgs = await getTgMessages(c, ctx);
  ...
  bot.command('address', async ctx => {
    ...
    try {
      const jwtList =
        (await c.env.KV.get<string[]>(`${CONSTANTS.TG_KV_PREFIX}:${userId}`, 'json')) || [];
      const { addressList } = await jwtListToAddressData(c, jwtList, msgs);
      return await ctx.reply(
        `${msgs.TgAddressListMsg}\n\n` +
  ...
  const queryMail = async (
    ...
    const db_address_id = await c.env.DB.prepare(`SELECT id FROM address where id = ? `)
      .bind(address_id)
      .first('id');
    if (!db_address_id) {
    ...
    const mailRow = await c.env.DB.prepare(
      `SELECT * FROM raw_mails where address = ? ` + ` order by id desc limit 1 offset ?`
    )
      .bind(queryAddress, mailIndex)
      .first<RawMailRow>();
    const raw = mailRow ? await resolveRawEmail(mailRow) : undefined;
    const mailId = mailRow?.id;
    const created_at = mailRow?.created_at;
    const { mail } = raw
      ? await parseMail(msgs, { rawEmail: raw }, queryAddress, created_at)
      ...
      return await ctx.editMessageText(mail || msgs.TgNoMailMsg, {
        ...Markup.inlineKeyboard([
          Markup.button.callback(
            msgs.TgPrevBtnMsg,
            `mail_${queryAddress}_${mailIndex - 1}`,
            mailIndex <= 0
          ),
          ...miniAppButtons,
