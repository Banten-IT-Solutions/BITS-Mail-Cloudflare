...
export const getJsonObjectValue = <T = any>(value: string | any): T | null => {
...
export const getSetting = async (
  ...
  try {
    const value = await c.env.DB.prepare(`SELECT value FROM settings where key = ?`)
      .bind(key)
      .first<string>('value');
    return value;
  } catch (error) {
...
export const saveSetting = async (c: Context<HonoCustomType>, key: string, value: string) => {
...
export const deleteSetting = async (c: Context<HonoCustomType>, key: string) => {
...
export const getSplitStringListValue = (value: any, demiliter: string = ','): string[] => {
...
export const getBooleanValue = (value: boolean | string | any): boolean => {
...
export const getIntValue = (value: number | string | any, defaultValue: number = 0): number => {
...
export const getStringArray = (value: string | string[] | undefined | null): string[] => {
  if (!value) {
    return [];
  }
      ...
      if (compressed) {
        try {
          ({ success } = await c.env.DB.prepare(
            ...
            .bind('admin@internal', toMail, compressed, message_id)
            .run());
        } catch (dbError) {
          ...
          if (errMsg.includes('raw_blob') || errMsg.includes('no such column')) {
            ...
            ({ success } = await c.env.DB.prepare(
              ...
              .bind('admin@internal', toMail, rawText, message_id)
              .run());
          } else {
            throw dbError;
          }
...
export const isGlobalTurnstileEnabled = (c: Context<HonoCustomType>): boolean => {
  return (
    getBooleanValue(c.env.ENABLE_GLOBAL_TURNSTILE_CHECK) &&
