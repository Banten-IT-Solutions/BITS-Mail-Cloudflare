...
export default {
  getConfig: async (c: Context<HonoCustomType>) => {
    return c.json({
      ...
      SUBDOMAIN_FORWARD_ADDRESS_LIST: utils.getJsonObjectValue<SubdomainForwardAddressList[]>(
        c.env.SUBDOMAIN_FORWARD_ADDRESS_LIST
      ),
      DEFAULT_DOMAINS: utils.getDefaultDomains(c),
      DOMAINS: utils.getDomains(c),
      ENABLE_CREATE_ADDRESS_SUBDOMAIN_MATCH: utils.getBooleanValue(
      ...
      DISABLE_ANONYMOUS_USER_CREATE_EMAIL: utils.getBooleanValue(
        c.env.DISABLE_ANONYMOUS_USER_CREATE_EMAIL
      ),
