...
async function getWebhookSettings(c: Context<HonoCustomType>): Promise<Response> {
  const settings =
    (await c.env.KV.get<WebhookSettings>(CONSTANTS.WEBHOOK_KV_ADMIN_MAIL_SETTINGS_KEY, 'json')) ||
    new WebhookSettings();
...
async function saveWebhookSettings(c: Context<HonoCustomType>): Promise<Response> {
  ...
  await c.env.KV.put(CONSTANTS.WEBHOOK_KV_ADMIN_MAIL_SETTINGS_KEY, JSON.stringify(settings));
