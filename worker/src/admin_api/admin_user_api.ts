...
export default {
  ...
  saveSetting: async (c: Context<HonoCustomType>) => {
    ...
    if (settings.enableMailVerify && settings.verifyMailSender) {
      ...
      if (!domains.includes(mailDomain)) {
        return c.text(
          `${msgs.VerifyMailDomainInvalidMsg} ${JSON.stringify(domains, null, 2)}`,
          400
        );
      }
    }
  ...
  createUser: async (c: Context<HonoCustomType>) => {
    ...
    try {
      ...
      const { success } = await c.env.DB.prepare(
        ...
        .bind(email, password, JSON.stringify(userInfo))
        .run();
      if (!success) {
  ...
  resetPassword: async (c: Context<HonoCustomType>) => {
    ...
    try {
      checkUserPassword(password);
      const { success } = await c.env.DB.prepare(`UPDATE users SET password = ? WHERE id = ?`)
  ...
  updateUserRoles: async (c: Context<HonoCustomType>) => {
    ...
    if (!role_text) {
      const { success } = await c.env.DB.prepare(`DELETE FROM user_roles WHERE user_id = ?`)
        .bind(user_id)
        .run();
      if (!success) {
    ...
    const { success } = await c.env.DB.prepare(
      ...
      .bind(user_id, role_text, role_text)
      .run();
    if (!success) {
      return c.text(msgs.FailedUpdateUserDefaultRoleMsg, 500);
    }
  ...
  bindAddress: async (c: Context<HonoCustomType>) => {
    ...
    const db_user_id =
      user_id ??
      (await c.env.DB.prepare(`SELECT id FROM users WHERE user_email = ?`)
        .bind(user_email)
        .first<number | undefined | null>('id'));
    const db_address_id =
      address_id ??
      (await c.env.DB.prepare(`SELECT id FROM address WHERE name = ?`)
        .bind(address)
        .first<number | undefined | null>('id'));
