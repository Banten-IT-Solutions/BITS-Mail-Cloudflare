...
const getAdminSendMailErrorMessage = (
  ...
  return Object.values(msgs).includes(message) ? message : msgs.OperationFailedMsg;
};
...
export const sendMailbyAdmin = async (c: Context<HonoCustomType>) => {
  ...
  const { from_name, from_mail, to_mail, to_name, subject, content, is_html } = reqJson;
  try {
    await sendMail(
      c,
      from_mail,
...
export const sendMailByBindingAdmin = async (c: Context<HonoCustomType>) => {
  ...
  const { from, to, subject, html, text, cc, bcc, replyTo, attachments, headers } = reqJson;
  if (!from || !to || !subject || (!html && !text)) {
    return c.text(msgs.InvalidInputMsg, 400);
  }
  const fromMail = typeof from === 'string' ? from : from?.email;
  const mailDomain =
    typeof fromMail === 'string' && fromMail.includes('@')
