...
async function saveIpBlacklistSettings(c: Context<HonoCustomType>): Promise<Response> {
  ...
  if (settings.fingerprintBlacklist.length > MAX_BLACKLIST_SIZE) {
    return c.text(
      `${msgs.BlacklistExceedsMaxSizeMsg}: fingerprintBlacklist (${MAX_BLACKLIST_SIZE})`,
      400
    );
  }
  ...
  if (settings.whitelist.length > MAX_BLACKLIST_SIZE) {
    return c.text(
      `${msgs.BlacklistExceedsMaxSizeMsg}: whitelist (${settings.whitelist.length}/${MAX_BLACKLIST_SIZE})`,
      400
    );
  }
  ...
  for (const pattern of settings.whitelist) {
    if (typeof pattern !== 'string') {
      return c.text(
        `${msgs.InvalidIpBlacklistSettingMsg}: whitelist element must be a string`,
        400
      );
    }
  ...
  await saveSetting(c, CONSTANTS.IP_BLACKLIST_SETTINGS_KEY, JSON.stringify(sanitizedSettings));
