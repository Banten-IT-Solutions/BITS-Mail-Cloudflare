 ...
 */
export const isSendMailEnabled = (c: Context<HonoCustomType>, mailDomain: string): boolean => {
  // Check resend token for domain or global
  const resendEnabled =
    c.env.RESEND_TOKEN || c.env[`RESEND_TOKEN_${mailDomain.replace(/\./g, '_').toUpperCase()}`];
...
export const isSendMailBindingEnabled = (
  ...
  const sendMailDomains = getStringArray(c.env.SEND_MAIL_DOMAINS).map(domain =>
    normalizeDomainValue(domain)
  );
...
const allowRandomSubdomainForDomain = (c: Context<HonoCustomType>, domain: string): boolean => {
...
const isCreateAddressSubdomainMatchEnvConfigured = (c: Context<HonoCustomType>): boolean => {
  return (
    c.env.ENABLE_CREATE_ADDRESS_SUBDOMAIN_MATCH !== undefined &&
...
export const getAddressCreationSettings = async (
  ...
  const value = await getJsonSetting<AddressCreationSettings>(
    c,
    CONSTANTS.ADDRESS_CREATION_SETTINGS_KEY
  );
...
export function updateAddressUpdatedAt(
  ...
  c.executionCtx.waitUntil(
    (async () => {
      try {
        await c.env.DB.prepare(`UPDATE address SET updated_at = datetime('now') where name = ?`)
          .bind(address)
          .run();
      } catch (e) {
...
const insertAddressRecord = async (
  ...
  try {
    const result = await c.env.DB.prepare(`INSERT INTO address(name, source_meta) VALUES(?, ?)`)
    ...
    if (message && message.includes('source_meta')) {
      const result = await c.env.DB.prepare(`INSERT INTO address(name) VALUES(?)`)
        .bind(address)
        .run();
      if (!result.success) {
...
export const newAddress = async (
  ...
  if (!domain && allowDomains.length > 0) {
    const createAddressDefaultDomainFirst = getBooleanValue(
      c.env.CREATE_ADDRESS_DEFAULT_DOMAIN_FIRST
    );
  ...
  const { effectiveEnabled: enableSubdomainMatch } =
  ...
  for (let attempt = 0; attempt < maxAttempts; attempt++) {
    ...
    try {
      ...
      const address_id = await c.env.DB.prepare(`SELECT id FROM address where name = ?`)
        .bind(address)
        .first<number>('id');
...
const checkNameBlockList = async (c: Context<HonoCustomType>, name: string): Promise<void> => {
...
export const deleteAddressWithData = async (
  ...
  if (!address_id) {
    address_id = await c.env.DB.prepare(`SELECT id FROM address where name = ?`)
      .bind(address)
      .first<number>('id');
  } else if (!address) {
    address = await c.env.DB.prepare(`SELECT name FROM address where id = ?`)
      .bind(address_id)
      .first<string>('name');
  }
  // check address again
  ...
  const { success: autoReplySuccess } = await c.env.DB.prepare(
    ...
    .bind(address)
    .run();
  const { success } = await c.env.DB.prepare(`DELETE FROM address WHERE name = ? `)
    .bind(address)
    .run();
  if (
    !success ||
...
export const handleListQuery = async (
  ...
  const { results } = await c.env.DB.prepare(resultsQuery)
...
export const commonParseMail = async (
...
export const commonGetUserRole = async (
  ...
  const role_text = await c.env.DB.prepare(`SELECT role_text FROM user_roles where user_id = ?`)
    .bind(user_id)
    .first<string | undefined | null>('role_text');
...
export async function sendWebhook(
  ...
  for (const key of Object.keys(formatMap)) {
    body = body.replace(
      new RegExp(`\\$\\{${key}\\}`, 'g'),
      JSON.stringify(formatMap[key as keyof WebhookMail]).replace(/^"(.*)"$/, '$1')
    );
  }
  ...
  if (!response.ok) {
    ...
    return {
      success: false,
      message: `send webhook error: ${response.status} ${response.statusText}`,
    };
  }
  return { success: true };
}
...
export async function triggerWebhook(
  ...
  // admin mail webhook
  const adminMailWebhookSettings = await c.env.KV.get<WebhookSettings>(
  ...
  // user mail webhook
  const adminSettings = await c.env.KV.get<AdminWebhookSettings>(
