  ...
  return whitelist.some(pattern => {
    ...
    if (looksLikeRegex(normalizedPattern)) {
      try {
        ...
        console.warn(
          `Whitelist regex "${normalizedPattern}" failed to parse: ${(error as Error).message}, treating as no-match`
        );
        return false;
      }
    }
...
export async function getIpBlacklistSettings(
  ...
  return await getJsonSetting<IpBlacklistSettings>(c, CONSTANTS.IP_BLACKLIST_SETTINGS_KEY);
}
...
function checkIpWhitelist(
  ...
  if (!reqIp) {
    console.warn(
      `Blocked request without cf-connecting-ip under whitelist mode for path: ${c.req.path}`
    );
...
function checkIpAsnBlacklist(
  ...
  if (settings.asnBlacklist && settings.asnBlacklist.length > 0) {
    ...
    if (asOrganization && isBlacklisted(asOrganization as string, settings.asnBlacklist, false)) {
      console.warn(
        `Blocked blacklisted ASN: ${asOrganization} (IP: ${reqIp}) for path: ${c.req.path}`
      );
...
async function checkDailyLimit(
  ...
  if (current_count && current_count >= dailyLimit) {
    console.warn(
      `Blocked IP ${reqIp} exceeded daily limit of ${dailyLimit} requests for path: ${c.req.path}`
    );
    return c.text(`IP=${reqIp} Exceeded daily limit of ${dailyLimit} requests`, 429);
  }

  // Increment counter with 24-hour expiration
  await c.env.KV.put(daily_count_key, ((current_count || 0) + 1).toString(), {
 ...
 */
export async function checkAccessControl(c: Context<HonoCustomType>): Promise<Response | null> {
  try {
