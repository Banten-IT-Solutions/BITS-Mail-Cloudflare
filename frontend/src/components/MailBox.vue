...
const data = computed(() => {
  ...
  return rawData.value.filter(mail => {
    ...
    const searchFields = [mail.subject || '', mail.text || '', mail.message || ''].map(field =>
      field.toLowerCase()
    );
...
const { t } = useI18n({
  messages: {
    ...
    id: {
      ...
      refresh: 'Refresh',
      attachments: 'Tampilkan Lampiran',
      downloadMail: 'Unduh Mail',
      pleaseSelectMail: 'Silakan pilih mail',
      emptyInbox: 'Kotak masuk Anda kosong',
      delete: 'Hapus',
      deleteMailTip: 'Apakah Anda yakin ingin menghapus mail?',
      reply: 'Balas',
      forwardMail: 'Teruskan',
      showTextMail: 'Tampilkan Mail Teks',
      showHtmlMail: 'Tampilkan Mail HTML',
      ...
      prevMail: 'Sebelumnya',
      nextMail: 'Berikutnya',
      keywordQueryTip: 'Filter halaman saat ini',
      query: 'Kueri',
    },
  },
});
...
const mailItemClass = row => {
  return curMail.value && row.id == curMail.value.id
    ? isDark.value
      ? 'overlay overlay-dark-backgroud'
      : 'overlay overlay-light-backgroud'
    : '';
};
              ...
              >
                ...
                <n-thing :title="row.subject">
                  <template #description>
                    <n-tag type="info"> ID: {{ row.id }} </n-tag>
                    <n-tag type="info">
                    ...
                    <n-tag v-if="showEMailTo" type="info">
                      <n-ellipsis style="max-width: 240px"> TO: {{ row.address }} </n-ellipsis>
                    </n-tag>
