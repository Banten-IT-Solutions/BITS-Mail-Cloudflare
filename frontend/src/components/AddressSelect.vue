...
const { jwt, settings, userJwt, isTelegram, openSettings, telegramApp } = useGlobalState();
...
const formatAddressLabel = address => {
  ...
  const domainLabel = openSettings.value.domains.find(d => d.value === domain)?.label;
...
const buildUserOptions = async () => {
  ...
  try {
    ...
    for (const row of results || []) {
      ...
      const option = {
        label,
        value: getOptionValue(key, 'user', String(row.id), address),
        address,
      };
      if (settings.value.address && address === settings.value.address) {
        addressValue.value = option.value;
...
const buildTelegramOptions = async () => {
  ...
  try {
    ...
    for (const row of data || []) {
      ...
      const option = {
        label,
        value: getOptionValue(key, 'tg', row.jwt, row.address),
        address: row.address,
      };
