    ...
    data.value = results.map(item => {
      try {
        ...
        if (data.version == 'v2') {
          ...
          item.to_mail = data?.personalizations
            ?.map(p => p.to?.map(t => t.email).join(','))
            .join(';');
          item.subject = data.subject;
      ...
      <div style="margin-bottom: 10px">
        ...
        <n-space v-else>
          <n-button
            ...
            @click="multiActionModeClick(true)"
            type="primary"
            tertiary
          >
            {{ t('multiAction') }}
              ...
              >
                ...
                <n-thing :title="row.subject">
                  <template #description>
                    ...
                    <n-tag v-if="showEMailFrom" type="info"> FROM: {{ row.address }} </n-tag>
                    <n-tag type="info"> TO: {{ row.to_mail }} </n-tag>
                  </template>
                </n-thing>
              </n-list-item>
            </n-list>
        ...
        <template #2>
          ...
          >
            <n-space>
              ...
              <n-tag type="info"> FROM: {{ curMail.address }} </n-tag>
              <n-tag type="info"> TO: {{ curMail.to_mail }} </n-tag>
