  ...
  messages: {
    en: {
      ...
      cannotForgotPassword:
        'Mail verification is disabled or register is disabled, cannot reset password, please contact administrator',
    ...
    id: {
      ...
      cannotForgotPassword:
        'Verifikasi email tidak diaktifkan atau pendaftaran tidak diaktifkan, tidak dapat mengatur ulang kata sandi, silakan hubungi administrator',
...
const sendVerificationCode = async () => {
  ...
  if (
    ...
    !currentCfToken &&
    userOpenSettings.value.enableMailVerify
  ) {
...
const oauth2Login = async clientID => {
  try {
    ...
    const res = await api.fetch(
      `/user_api/oauth2/login_url?clientID=${clientID}&state=${userOauth2SessionState.value}`
    );
      ...
      <n-tab-pane v-if="userOpenSettings.enable" name="signup" :tab="t('register')">
        <n-form>
          ...
          <Turnstile
            ...
            v-if="userOpenSettings.enableMailVerify"
