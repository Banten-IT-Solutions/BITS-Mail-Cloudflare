...
const { userSettings, userJwt, userOpenSettings } = useGlobalState();
  ...
  messages: {
    en: {
      ...
      fetchUserSettingsError:
        'Login password is invalid or account not exist, it may be network connection issue, please try again later.',
    },
    id: {
      currentUser: 'Pengguna Login Saat Ini',
      fetchUserSettingsError:
        'Kata sandi login tidak valid atau akun tidak ada, mungkin ada masalah koneksi jaringan, silakan coba lagi nanti.',
    },
  },
});
  ...
  <div>
    ...
    <div v-else-if="userSettings.user_email">
      <n-alert type="success" :show-icon="false" :bordered="false">
        <span>
          <b
            >{{ t('currentUser') }} <b>{{ userSettings.user_email }}</b></b
          >
        </span>
      </n-alert>
    </div>
