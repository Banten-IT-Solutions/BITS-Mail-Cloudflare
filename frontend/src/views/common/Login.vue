...
const { jwt, loading, openSettings, showAddressCredential, userSettings, addressPassword } =
  useGlobalState();
...
const { locale, t } = useI18n({
  messages: {
    en: {
      ...
      bindUserInfo:
        'Logged in user, login without binding email or create new email address will bind to current user',
    ...
    id: {
      login: 'Masuk',
      loginAndBind: 'Masuk dan Ikat',
      pleaseGetNewEmail:
        'Silakan masuk atau klik tombol "Dapatkan Email Baru" untuk mendapatkan alamat email baru',
      getNewEmail: 'Buat Email Baru',
      getNewEmailTip1: 'Masukkan email yang ingin Anda gunakan. Hanya mengizinkan: ',
      getNewEmailTip2: 'Meninggalkan kosong akan menghasilkan alamat email acak.',
      getNewEmailTip3: 'Anda dapat memilih domain dari daftar dropdown.',
      credential: 'Kredensial Alamat Email',
      ok: 'OK',
      generateName: 'Buat Nama Palsu',
      help: 'Bantuan',
      credentialInput: 'Silakan masukkan Kredensial Alamat Email',
      bindUserInfo:
        'Pengguna yang masuk, login tanpa mengikat email atau membuat alamat email baru akan mengikat ke pengguna saat ini',
      bindUserAddressError: 'Kesalahan saat mengikat alamat email ke pengguna',
      autoGeneratedName: 'Nama yang dihasilkan otomatis',
      passwordLogin: 'Login dengan Kata Sandi',
      credentialLogin: 'Login dengan Kredensial',
      email: 'Email',
      password: 'Kata Sandi',
      emailPasswordRequired: 'Email dan kata sandi diperlukan',
      enableRandomSubdomain: 'Gunakan Subdomain Acak',
      randomSubdomainTip:
        'Saat diaktifkan, alamat yang dibuat akan menggunakan subdomain acak. Alamat subdomain direkomendasikan untuk menerima saja.',
    },
  },
});
...
const generateName = async () => {
  try {
    ...
    emailName.value = faker.internet
      .email()
      .split('@')[0]
      .replace(/\s+/g, '.')
      .replace(/\.{2,}/g, '.')
