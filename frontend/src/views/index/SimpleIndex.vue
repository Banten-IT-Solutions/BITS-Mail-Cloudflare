...
const { jwt, settings, useSimpleIndex, showAddressCredential, openSettings, loading } =
  useGlobalState();
...
const fetchMails = async () => {
  ...
  try {
    const { results, count } = await api.fetch(
      `/api/mails?limit=1&offset=${currentPage.value - 1}`
    );
...
<template>
  <div class="center">
    ...
    <div v-else>
      ...
      <!-- 账户设置卡片 -->
      <n-card
        v-if="showAccountSettingsCard"
        :bordered="false"
        embedded
        closable
        @close="showAccountSettingsCard = false"
