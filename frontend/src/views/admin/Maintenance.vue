...
const { t } = useI18n({
  messages: {
    en: {
      ...
      save: 'Save',
      cronTip:
        'Enable cron cleanup, need to configure [crons] in worker, please refer to the document, setting 0 days means clear all',
      basicCleanup: 'Basic Cleanup',
      customSqlCleanup: 'Custom SQL Cleanup',
      customSqlTip:
        'Add custom DELETE SQL statements for scheduled cleanup. Only single DELETE statement is allowed per entry.',
      addCustomSql: 'Add Custom SQL',
      sqlName: 'Name',
      sqlStatement: 'SQL Statement (DELETE only)',
      sqlNamePlaceholder: 'e.g. Clean old logs',
      sqlPlaceholder:
        "e.g. DELETE FROM raw_mails WHERE source GLOB '*{'@'}example.com' AND created_at < datetime('now', '-3 day')",
      deleteCustomSql: 'Delete',
    },
    id: {
      tip: 'Silakan masukkan jumlah hari',
      mailBoxLabel: 'Bersihkan kotak masuk sebelum N hari',
      mailUnknowLabel: 'Bersihkan email tak dikenal sebelum N hari',
      sendBoxLabel: 'Bersihkan kotak kiriman sebelum N hari',
      addressCreateLabel: 'Bersihkan alamat yang dibuat sebelum N hari',
      inactiveAddressLabel: 'Bersihkan alamat tidak aktif sebelum N hari',
      unboundAddressLabel: 'Bersihkan alamat tidak terikat sebelum N hari',
      emptyAddressLabel: 'Bersihkan alamat kosong sebelum N hari',
      cleanupNow: 'Bersihkan sekarang',
      autoCleanup: 'Pembersihan otomatis',
      cleanupSuccess: 'Pembersihan berhasil',
      saveSuccess: 'Penyimpanan berhasil',
      save: 'Simpan',
      cronTip:
        'Aktifkan pembersihan cron, perlu mengonfigurasi [crons] di worker, lihat dokumen, pengaturan 0 hari berarti hapus semua',
      basicCleanup: 'Pembersihan Dasar',
      customSqlCleanup: 'Pembersihan SQL Khusus',
      customSqlTip:
        'Tambahkan pernyataan SQL DELETE khusus untuk pembersihan terjadwal. Hanya satu pernyataan DELETE yang diperbolehkan per entri.',
      addCustomSql: 'Tambahkan SQL Khusus',
      sqlName: 'Nama',
      sqlStatement: 'Pernyataan SQL (hanya DELETE)',
      sqlNamePlaceholder: 'mis. Bersihkan log lama',
      sqlPlaceholder:
        "mis. DELETE FROM raw_mails WHERE source GLOB '*{'@'}example.com' AND created_at < datetime('now', '-3 day')",
      deleteCustomSql: 'Hapus',
    },
  },
});
    ...
    <n-card :bordered="false" embedded>
      ...
      <n-tabs type="segment" style="margin-top: 16px">
        <n-tab-pane name="basic" :tab="t('basicCleanup')">
          <n-form :model="cleanupModel">
            ...
            <n-form-item-row :label="t('mailUnknowLabel')">
              ...
              <n-input-number
                v-model:value="cleanupModel.cleanUnknowMailsDays"
                :placeholder="t('tip')"
              />
            ...
            <n-form-item-row :label="t('sendBoxLabel')">
              ...
              <n-input-number
                v-model:value="cleanupModel.cleanSendBoxDays"
                :placeholder="t('tip')"
              />
            ...
            <n-form-item-row :label="t('addressCreateLabel')">
              ...
              <n-input-number
                v-model:value="cleanupModel.cleanAddressDays"
                :placeholder="t('tip')"
              />
            ...
            <n-form-item-row :label="t('inactiveAddressLabel')">
              ...
              <n-input-number
                v-model:value="cleanupModel.cleanInactiveAddressDays"
                :placeholder="t('tip')"
              />
            ...
            <n-form-item-row :label="t('unboundAddressLabel')">
              ...
              <n-input-number
                v-model:value="cleanupModel.cleanUnboundAddressDays"
                :placeholder="t('tip')"
              />
            ...
            <n-form-item-row :label="t('emptyAddressLabel')">
              ...
              <n-input-number
                v-model:value="cleanupModel.cleanEmptyAddressDays"
                :placeholder="t('tip')"
              />
        ...
        <n-tab-pane name="custom_sql" :tab="t('customSqlCleanup')">
          ...
          <n-space vertical>
            <n-card
              v-for="(item, index) in cleanupModel.customSqlCleanupList"
              :key="item.id"
              size="small"
            >
              <n-space vertical>
                <n-space align="center">
                  ...
                  <n-input
                    ...
                    :placeholder="t('sqlNamePlaceholder')"
                    style="width: 200px"
                  />
