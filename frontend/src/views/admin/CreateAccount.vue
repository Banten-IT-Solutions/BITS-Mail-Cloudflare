...
const { t } = useI18n({
  messages: {
    en: {
      ...
      randomSubdomainTip:
        'When enabled, the created address will use a random subdomain. Subdomain addresses are recommended for receiving only.',
    },
    id: {
      address: 'Alamat',
      ...
      randomSubdomainTip:
        'Saat diaktifkan, alamat yang dibuat akan menggunakan subdomain acak. Alamat subdomain disarankan hanya untuk menerima.',
    },
  },
});
  ...
  <div class="center">
    <n-modal v-model:show="showReultModal" preset="dialog" :title="t('addressCredential')">
      ...
      <n-card embedded v-if="addressPassword">
        ...
        <p>
          {{ t('addressPassword') }}: <b>{{ addressPassword }}</b>
        </p>
      </n-card>
      <n-card embedded>
        <n-collapse>
