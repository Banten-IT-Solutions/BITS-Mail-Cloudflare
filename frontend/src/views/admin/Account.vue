...
const { loading, adminTab, openSettings, adminMailTabAddress, adminSendBoxTabAddress } =
  useGlobalState();
  ...
  messages: {
    en: {
      ...
      addressCredentialTip:
        'Please copy the Mail Address Credential and you can use it to login to your email account.',
      delete: 'Delete',
    ...
    id: {
      ...
      source_meta: 'Sumber',
      showCredential: 'Tampilkan Kredensial Alamat Email',
      addressCredential: 'Kredensial Alamat Email',
      addressCredentialTip:
        'Silakan salin Kredensial Alamat Email dan Anda dapat menggunakannya untuk masuk ke akun email Anda.',
      delete: 'Hapus',
      deleteTip: 'Anda yakin ingin menghapus email ini?',
      deleteAccount: 'Hapus Akun',
      viewMails: 'Lihat Email',
      viewSendBox: 'Lihat Kotak Kiriman',
      itemCount: 'itemCount',
      query: 'Kueri',
      addressQueryTip: 'Biarkan kosong untuk mengkueri semua alamat',
      clearInbox: 'Bersihkan Kotak Masuk',
      clearSentItems: 'Bersihkan Kotak Terkirim',
      clearInboxTip: 'Anda yakin ingin membersihkan kotak masuk untuk email ini?',
      clearSentItemsTip: 'Anda yakin ingin membersihkan item terkirim untuk email ini?',
      actions: 'Aksi',
      success: 'Berhasil',
      ...
      multiDelete: 'Hapus Beberapa',
      multiDeleteTip: 'Anda yakin ingin menghapus alamat yang dipilih?',
      multiClearInbox: 'Bersihkan Kotak Masuk Beberapa',
      multiClearInboxTip: 'Anda yakin ingin membersihkan kotak masuk untuk alamat yang dipilih?',
      multiClearSentItems: 'Bersihkan Item Terkirim Beberapa',
      multiClearSentItemsTip:
        'Anda yakin ingin membersihkan item terkirim untuk alamat yang dipilih?',
    },
  },
});
...
const multiActionClearInbox = async () => {
  await executeBatchOperation({
    ...
    apiCall: id =>
      api.fetch(`/admin/clear_inbox/${id}`, {
        method: 'DELETE',
      }),
...
const multiActionClearSentItems = async () => {
  await executeBatchOperation({
    ...
    apiCall: id =>
      api.fetch(`/admin/clear_sent_items/${id}`, {
        method: 'DELETE',
      }),
...
const columns = computed(() => [
  ...
  },
  {
    ...
    render(row) {
      ...
      if (
        ipv4Regex.test(val) ||
        (val.includes(':') && ipv6Regex.test(val) && !val.startsWith('tg:'))
      ) {
        return h(
          'a',
          {
...
<template>
  <div style="margin-top: 10px">
    ...
    <n-modal
      v-model:show="showMultiActionModal"
      preset="dialog"
      :title="multiActionTitle"
      negative-text="OK"
    >
