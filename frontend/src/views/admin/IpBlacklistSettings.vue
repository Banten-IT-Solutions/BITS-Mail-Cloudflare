  ...
  messages: {
    en: {
      ...
      enable_whitelist_tip:
        'Strict mode: ONLY IPs matching the whitelist can access rate-limited APIs. All other IPs will be denied.',
      ip_whitelist: 'IP Whitelist Patterns',
      ip_whitelist_placeholder:
        'Exact IP (e.g., 1.2.3.4) or anchored regex (e.g., ^192\\.168\\.1\\.\\d+$)',
      tip_whitelist:
        'IP Whitelist: Strict allowlist — plain entries must be EXACT IP matches (no substring). Use anchored regex (^...$) for ranges. Whitelisted IPs skip blacklist checks.',
      whitelist_empty_warning:
        'IP whitelist is enabled but the list is empty. This is ignored by the server to prevent lockout. Please add at least one entry before enabling.',
      ...
      tip_ip:
        'IP Blacklist: Supports text matching (e.g., "192.168.1") or regex (e.g., "^10\\.0\\.0\\.5$").',
      tip_asn: 'ASN Organization: Block by ISP/provider. Case-insensitive text matching or regex.',
      tip_fingerprint:
        'Browser Fingerprint: Block by browser fingerprint. Supports exact matching or regex patterns.',
      tip_daily_limit:
        'Daily Limit: Restrict the maximum number of requests per IP address per day (1-1000000).',
      tip_scope:
        'Applies to: Create Address, Send Mail, External Send Mail API, User Registration, Verify Code',
    ...
    id: {
      ...
      enable_whitelist_tip:
        'Mode ketat: HANYA IP yang cocok dengan daftar putih yang dapat mengakses API yang dibatasi. Semua IP lain akan ditolak.',
      ip_whitelist: 'Pola Daftar Putih IP',
      ip_whitelist_placeholder:
        'IP tepat (mis. 1.2.3.4) atau regex berjangkar (mis. ^192\\.168\\.1\\.\\d+$)',
      tip_whitelist:
        'Daftar Putih IP: allowlist ketat — entri biasa harus cocok dengan IP yang SAMA PERSIS (bukan substring). Gunakan regex berjangkar (^...$) untuk rentang. IP yang ada di daftar putih akan melewati pemeriksaan daftar hitam.',
      whitelist_empty_warning:
        'Daftar putih IP aktif tetapi daftarnya kosong. Ini diabaikan oleh server untuk mencegah terkunci. Tambahkan setidaknya satu entri sebelum mengaktifkannya.',
      ...
      tip_ip:
        'Daftar Hitam IP: Mendukung pencocokan teks (mis. "192.168.1") atau regex (mis. "^10\\.0\\.0\\.5$").',
      tip_asn:
        'Organisasi ASN: Blokir berdasarkan ISP/provider. Pencocokan teks tidak peka huruf besar/kecil atau regex.',
      tip_fingerprint:
        'Browser Fingerprint: Blokir berdasarkan fingerprint browser. Mendukung pencocokan tepat atau pola regex.',
      tip_daily_limit:
        'Batas Harian: Batasi jumlah maksimum request per alamat IP per hari (1-1000000).',
      tip_scope:
        'Berlaku untuk: Buat Alamat, Kirim Email, API Kirim Email Eksternal, Registrasi Pengguna, Verifikasi Kode',
