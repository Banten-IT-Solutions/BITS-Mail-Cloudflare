...
const { t } = useI18n({
  messages: {
    ...
    id: {
      ...
      userEmailFormatTip:
        'Gunakan regex untuk mengubah email. Contoh: ^(.+)@old\\.com$ dengan $1@new.com',
  ...
  <div class="center">
    ...
    <n-card :bordered="false" embedded style="max-width: 600px">
      ...
      <n-collapse default-expanded-names="1" accordion :trigger-areas="['main', 'arrow']">
        <n-collapse-item
          v-for="(item, index) in userOauth2Settings"
          :key="index"
          :title="item.name"
        >
          ...
          <n-form :model="item">
            ...
            <n-form-item-row label="Access Token Params Format" required>
              <n-select
                v-model:value="item.accessTokenFormat"
            ...
            <n-form-item-row :label="t('enableMailAllowList')">
              <n-input-group>
                ...
                <n-select
                  v-model:value="item.mailAllowList"
                  v-if="item.enableMailAllowList"
                  filterable
                  multiple
                  tag
                  style="width: 80%"
